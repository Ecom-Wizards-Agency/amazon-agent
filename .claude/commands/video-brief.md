...
Build keyword-driven Sponsored Brands video briefs for editors. Do not duplicate logic here. Route into the `amazon-sponsored-brands-video-briefs` skill.
...
2. **Load the skill.** `skills/amazon-sponsored-brands-video-briefs/SKILL.md` is the source of truth. Read its three references before scripting. Copy `tools/sb-video-briefs/config.TEMPLATE.json` to a local `config.<client>-<market>-<product-line>.json` on first run (client configs stay local, gitignored).
...
8. **Campaign structure.** State the naming: one campaign per keyword, one ad group per angle (the batch) named `Angle N - <name>`. Stop there; campaign creation routes to amazon-sponsored-products-bulk-files.
