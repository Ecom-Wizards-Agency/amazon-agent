...
6. **Deliver + handoff.** Review the `.xlsx`, then copy **only the final Excel** to the client's Google Drive keyword folder per `AGENTS.md`: `<your-gdrive-mount>/Geteilte Ablagen/Ecom Wizards/01_Client Sheets/<Client>/Keyword Research/<Country>/` for multi-country clients, or directly under `…/<Client>/Keyword Research/` for single-country clients. Verify a byte-identical MD5. **Google Drive only — do NOT copy to pCloud** (the file becomes a Google Sheet there anyway). POE/DataDive raw files + the manifest stay out (embedded in the sheet / kept local). Surface the auto-generated manifest, Obsidian handoff note, and copy-ready Claude/Codex prompt.
