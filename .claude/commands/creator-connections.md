...
5. Run the requested mode. Match product in the order ASIN/product URL, campaign context, explicit product name. For MCF, require the full `preflight` to `reserve-mcf` to populated-screen `verify-mcf` to `confirm-mcf` chain. Use `cancel-mcf` only for an evidenced definitive failure; uncertain outcomes stay locked for reconciliation. Update the tracker, append the non-PII action log, and refresh the daily action queue.
