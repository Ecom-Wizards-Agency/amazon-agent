---
name: amazon-regulated-product-appeals
description: "Build evidence-controlled appeal packs for serious regulated-product suppressions, repeated denials, claim issues, and coordinated catalog corrections."
---
...
5. Route catalog correction work to `amazon-catalog` or `amazon-flatfilepro`. Route case wording to `amazon-communications`.
6. Generate the selected documents as DOCX masters, then deliver each one as a Google Doc with `python3 tools/gdrive-deliver/deliver.py <docx> "<drive folder>"`. The Doc is where the pack gets edited and reviewed; the PDF Amazon receives is downloaded from it at submission time (File > Download > PDF). Do not render PDFs.
