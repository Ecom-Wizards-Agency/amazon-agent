...
description: Use for end-to-end Amazon ad/sales audits from downloaded files (ads bulk + Business Report + multi-ASIN SQP + DataDive niche) into the branded MASTER workbook and operator narrative. Trigger on requests like ad audit, sales audit, or account audit for a client. For AdLabs-managed accounts use `amazon-adlabs-audit` instead.
---
...
7. **Write narrative** into the pre-filled scaffold per the playbook (voice, lean, Problems + Growth Levers). Run the playbook's **seven analytical checks** first (scale calibration, mechanism-not-metric, contradiction hunt, uniqueness test, shopper's-eye test, read the call notes, own the Buy Box): they are what separates an audit from a metrics dump, and every one of them is a gap the data already contains. Checks 6 and 7 are non-negotiable and are detailed below. The build also emits the **standard figure set** (`build_figures.py`: rank movement (the standard rank chart, from a Rank Radar payload), rank distribution, visibility vs competition, reviews vs price, branded vs generic, brand-name leak) next to the `.md` and the scaffold references them; add screenshots inline the same way, as `![caption](file.png)` (paths relative to the `.md`). The build renders the branded deliverable from this `.md`.
8. **Deliver** the MASTER `.xlsx` + the **branded audit `.docx`** to the client's Google Drive audit folder (the `.pdf` is optional; default is **docx-only** per operator preference, render/send a PDF only when asked). Confirm with the operator before a prospect sees it. **Uploading the binaries:** the Google Drive MCP `create_file` only takes content inline as base64, so a ~1 MB file is ~1.5 M tokens and cannot be emitted in a tool call (it fails). Copy the files into the local **Google Drive for Desktop** mount instead (`~/Library/CloudStorage/GoogleDrive-<acct>/…/04_Sales Audits/<Client>/`, note the localized "Geteilte Ablagen" = Shared drives) and Drive syncs them up; verify with `search_files parentId=<folder>`. Delete/replace by operating on the mount file. Only the MASTER `.xlsx` + `.docx` go in the client folder, never raw source files.
...
metrics, Ink-header tables, the approved V2 running header/footer, and smart page-break hygiene (no
dangling single-sentence page ends; KPI/table/figure never split). `build_audit.py` calls it automatically;
it falls back to plain `md_to_docx` if brand assets/Chrome are missing.
...
- **Font is Inter** (the website typeface). The brand-guide PDF lists Geist primary / Inter fallback, but the
  site (and these docs) uses Inter.
- **Content pages:** full black Ecom Wizards lockup at header left; uppercase report label plus month/year
  at header right. Footer is text only: report/client at left, `page X of Y` centered, website at right.
  Use Mist `#9AA5B4`. Never show the standalone rocket mark in the footer. Keep the cover free of duplicated
  running furniture and visually inspect every rendered page before delivery.
...
- **The A4 `.docx` is the primary deliverable** and opens in Google Docs preserving layout (upload with
  conversion disabled; a *native* Google Doc conversion breaks the full-bleed cover + KPI cards + font). The
  `.pdf` is optional: default is docx-only per operator preference, render one only when a send-ready PDF is asked for.
...
- **Audit only campaigns/targets with >0 spend in the window; treat zero-spend as dead weight, not a peer.** A campaign that spent $0 in the timeframe is an old, outdated campaign that is no longer used. It is not part of the live account, so never compare, grade, or ACOS-rank it against spending campaigns. The money splits already spend-weight (zero rows contribute zero), so no math changes; what changes is framing. Report the count of zero-spend / long-paused campaigns as a cleanup finding (structure hygiene), and keep every performance comparison on the spending set. On a large account the paused/zero-spend tail can be most of the campaign count.
...
- **Before writing: read the call notes, and check the Buy Box.** These are playbook checks 6 and 7 and they are the two that were learned the hard way. Search the Notion meeting-notes DB for the brand (match on people/product, not just the name: brands rebrand mid-engagement) so the audit reflects what the client actually told us their problem is. Then check Buy Box share, active seller count and price BEFORE grading ads: **SP only serves while you hold the featured offer**, so a hijacked listing caps spend and corrupts ACOS. Pull the Business Report **week by week** for this (`run.mjs business --start/--end` per week; there is no granularity flag) because a multi-week average hides a collapse.
- **Report-definition traps** (playbook, "Report-definition traps"): the industry's "SQP 24-hour attribution window" is NOT documented by Amazon (the rule is origination from the search results page); DataDive `price` is the Buy Box price and may be a hijacker's, so take the brand's price from BR ASP (sales ÷ units); and "page 1" means rank ≤15 in `analyze_audit` but ≤10 in `build_figures`, so pin prose to the shipped figure's definition. Verify report definitions first-party via the debug CDP Chrome (help article `G8J4CB5ZBF3NX7TP`, `brand-analytics/metric-glossary`), never from blogs.
- **Narrative opens organic-first** (playbook skeleton): organic reality → BR → SQP → DataDive → ads. An active Buy Box/hijacker problem outranks even that: it is the one thing costing money today. Read base bid × placement multiplier on the big campaigns (the product-page-bleed mechanism), check live titles against the ≤75-char rule (2026-07-27), and treat 4.5 stars as the visual review target (4.2/4.3 render the same). See the playbook's "Standard operator plays".
...
- **Reading Amazon bulk `.xlsx` (can be 100k+ rows; a real account hit ~288k rows on the SP Campaigns sheet alone, ~355 MB decompressed).** The analyzer streams it: `openpyxl` `read_only=True` + bounded `iter_rows` (`analyze_audit.py` `sheet_io`). Amazon writes a bogus `A1:A1` sheet `<dimension>`, and openpyxl 3.1.5 clips the read to it on **both** axes; `reset_dimensions=True` does **not** override it. The working fix is explicit bounds on both axes: read the header with a generous `max_col`, trim trailing empty cells to the true width, then stream the data with `min_row=2` + a large `max_row` at that width (the stream still stops at the real last row). Results are memoized (SB sheets are read up to 3x). Never open the bulk with `read_only=False`: per-cell `.cell()` access over ~288k rows ran ~40 min at multi-GB RSS with OOM risk. If a build hangs for minutes on a large account, this is why.
- **Source-file hygiene (standing rule):** the raw Amazon exports (ads bulk, Business Report, SQP) staged under `downloads/{client}/` are the retained working copy (that folder is gitignored, so it stays local). Once they are staged there, clear the same downloaded files from the browser's `~/Downloads`. Do NOT archive the raw exports to a Drive subfolder by default; the repo copy is enough. The client's Drive audit folder holds only the deliverables (MASTER `.xlsx` + branded `.docx`/`.pdf`), not raw source files. Keep only the files the config's `inputs{}` references; delete `--verbose` byproducts (`*.raw.json`) and any downloaded reports the build did not consume.
