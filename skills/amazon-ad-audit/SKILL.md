...
7. **Write narrative** into the pre-filled scaffold per the playbook (voice, lean, Problems + Growth Levers). Run the playbook's **seven analytical checks** first (scale calibration, mechanism-not-metric, contradiction hunt, uniqueness test, shopper's-eye test, read the call notes, own the Buy Box): they are what separates an audit from a metrics dump, and every one of them is a gap the data already contains. Checks 6 and 7 are non-negotiable and are detailed below. The build also emits the **standard figure set** (`build_figures.py`: rank distribution, visibility vs competition, reviews vs price, branded vs generic, brand-name leak) next to the `.md` and the scaffold references them; add screenshots inline the same way, as `![caption](file.png)` (paths relative to the `.md`). The build renders the branded deliverable from this `.md`.
