...
**Map columns by attribute name from the attribute row.** Never map by an index
...
- `Listing Action` = Create or Replace (Full Update); `Parentage Level` = Parent;
  ...
  tab for that product type; deprecated themes are labeled, do not use them).
...
- Exactly these fields, nothing else echoed: SKU, Product Type, `Listing Action` =
  ...
  (e.g. `size…value`). That value must be distinct across the family's children.
