...
- **AdLabs push variant:** for AdLabs-managed accounts the operator may ask to push instead of upload. That follows the `amazon-audit` write policy verbatim (explicit per-write lift in the current chat, batch-by-batch what-will-change summary, tags). Build the file first anyway; it is the reviewable artifact.
