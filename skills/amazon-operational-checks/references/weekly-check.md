...
8. Read the precomputed Keepa fee and package findings through the market-signals store's read CLI, not by parsing the state file. From the Wizards AI checkout (`~/os/wizards-ai/market_state_read.py`) run `python3 market_state_read.py since <date of the previous successful weekly run> --family fees,package`. It returns open and resolved rows, so a change that appeared and resolved between two weekly runs is still seen. The store keeps resolved fee and package rows for 35 days. If the previous successful weekly run is older than that, the window is incomplete: report it as a blocker rather than treating what is left as the full week's changes. Four issue types belong to this check: `fba_fee_changed`, `referral_fee_changed`, `package_dimensions_changed`, `package_weight_changed`. Never fetch market data yourself in this check.
...
Never run `amazon-fba-inventory-planning` or submit a shipment reconciliation request as part of this check.
