...
3. Step 0 of every run: re-check open findings before looking for new issues. Read them through the ledger's compact projection for this run's region, and one finding in full only by its key; never dump `{findings_ledger_path}` into the transcript. See `references/check-sequence.md`.
