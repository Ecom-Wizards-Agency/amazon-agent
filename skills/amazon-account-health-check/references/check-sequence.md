...
Step 0 of every run, before any new checks: re-check open findings. Read them with `{findings_projection_command}` - one JSON array of the open findings for this run's region, carrying each finding's key, account, marketplace, scope, issue type, summary, severity, disposition, owner, who it waits on and since when, linked task and case, impact, current state, deadline, last movement and next step - plus the open follow-up tasks it links for in-scope accounts. Never open, print or serialize `{findings_ledger_path}` itself: it is mostly closed history and dumping it spends the context the sweep needs. When one finding needs its full notes and evidence, ask that same command for the single key. A scheduled runner may pin its own exact form of the command in its instructions; that form wins. Every open finding gets re-verified in Seller Central during its account's check and re-disposed (see dispositions-and-ledger.md) - the run starts by answering "what was still open yesterday, and what is its status today?", not by looking for new issues.
