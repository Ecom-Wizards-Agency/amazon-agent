...
def write_output(path: Path, rows_by_sku: OrderedDict[str, dict[str, str]], headers: list[str]) -> None:
...
def main(argv: list[str]) -> int:
    ...
    write_output(args.output, rows_by_sku, headers)
    print(f"Wrote {len(rows_by_sku)} rows to {args.output}")
    return 0
