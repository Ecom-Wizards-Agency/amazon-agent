...
description: Run and verify a standardized Amazon client Day 0 onboarding. Use for Amazon onboarding access preflights, new-client account baselines, FBA disposal protection, Brand Tailored Promotion setup plans, approval-gated onboarding changes, onboarding verification, and the commands "Check Amazon onboarding access", "Run Amazon onboarding setup", "Approve Amazon onboarding changes", or "Verify Amazon onboarding".
---
...
2. Store run artifacts under `output/{client}/onboarding/{run_id}/` and screenshots under `evidence/{client}/onboarding/{run_id}/`. Never commit client manifests or evidence.
3. Use live Seller Central as the authority for permissions, settings, promotion eligibility, audience names, limits, and fees. Search first-party Amazon Help when a current rule is needed.
4. Set `run.mode` to `LIVE`, validate the manifest, and stamp its computed fingerprint before showing the change batch:
...
6. Record the finished work in `Clients/{Name}/Runs/YYYY-MM-DD-amazon-onboarding.md`. Update `Amazon Ops.md` only with verified durable facts.
