...
- Establish a baseline only. Hand recurring exceptions to `amazon-operations-review` and full reshipment planning to `amazon-inventory-planning`.
...
- Do not perform a deep PPC audit or change campaigns. Create a separate `amazon-audit` or `amazon-ads` task when needed.
