...
1. Use the `listing-optimization` or `health-claims-check` mode in `amazon-seo` for writing, semantic/Alexa/Rufus logic, and compliance posture.
...
- DataDive complete keyword-pool CSV filtered locally at `1% Min Rel.` from the three read-only endpoints below.
...
- **If you do fall back to a UI export, do not trust the download event.** In
  ...
  directory, snapshot that directory before the click, then poll for a new
  `niche-{id}-data*.zip`. Validate timestamp, size, ZIP members, headers, and row
  counts before accepting it. Cross-check against `get_niche_competitors`
  (`numVisibleKeywords`/`totalSvOfVisibleKeywords` for the visible set,
  `numKeywords`/`totalSvOfKeywords` for the full pool). Prefer the three read-only
...
- After the canonical inputs pass preflight and validation, register duplicate/raw intermediate downloads as `reproducible` (never the canonical contract paths). The weekly artifactctl job owns delayed quarantine.
...
Delivery is part of the build, not a separate step: set `inputs.drive_folder` in the client config to that country folder (Drive folder id, URL, or mounted path) and `build_keyword_workbook.py` delivers every QA-passed build as a native Google Sheet through `tools/gdrive-deliver/deliver.py`, writing a receipt next to the manifest. A build with a FAIL check is never delivered. `--no-deliver 1` builds without delivering; a missing `drive_folder` means build-only, which is the fallback when the client's Keyword Research folder has not been located yet. After the client has seen the Sheet, edit it in place instead of re-delivering (see Google Drive Delivery in `AGENTS.md`).

Rebuilds update the delivered Sheet in place: after the first delivery, set `inputs.drive_sheet_id` in the client config to the delivered Sheet's id (the build prints it as a hint; it is the receipt's `remote_id`). From then on a QA-passed rebuild goes through `tools/gdrive-deliver/update_sheet.py` rather than `deliver.py`: the file id, comments, links, and cell formatting survive, each workbook tab is cleared and rewritten with live formulas, and tabs that exist only in the Sheet are left untouched and reported as `tabs_orphaned`. A build with a FAIL check never updates the Sheet either. Leave `drive_sheet_id` empty for a first delivery into a new folder or country, and never point it at a Sheet the client did not receive from this workflow.
...
- `seo_identity` block present with the consumer-facing brand and recognizable product name. Both phrases must appear in the <=75-character title and description. Client/account name is never accepted as an implicit consumer brand.
...
- **Semantic / Alexa AI direction row present and non-empty, with traceable POE use**: its `poe_evidence` must include at least one POE Search Terms signal and at least one POE Reviews or Returns signal, each tied to a copy decision. Importing POE tabs without using them in the SEO does not pass.
...
- **Regulated-category evidence gate**: `checked=true` alone does not pass. Require a review date, a readable claims-check evidence file, non-empty per-claim decisions, and a visible Claim Guardrail row. `compliance.claims_audit` verdict=`prohibited` terms auto-merge into `triage.claim_tokens`.
