...
| Daily account health sweep / action queue, alert-source triage, daily update thread, disposition-routed follow-up tasks | `amazon-account-health-check` | Local account-health skill, findings ledger, configured alert source, Seller Central Account Health, task-tracker schema |
...
| Capture live listing copy (title/bullets/link) for anchor + competitors; replace ZeroWork scrape | `amazon-listing-capture` | Connected-browser extractor `tools/listing-capture/`, ZeroWork spec, feeds the keyword-workbook ASINs tab |
