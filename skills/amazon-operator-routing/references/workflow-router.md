...
| SOP bug report or SOP draft | `amazon-sop-maintenance`, source SOP/help file, current docs/UI evidence | `/bug`, `/create-sop`, `outdated SOP`, `broken SOP link`, `wrong SOP steps`, `new SOP draft` | Local report/draft under `output/{client-or-brand-or-general}/sop-maintenance/`; evidence under `evidence/{client-or-brand-or-general}/sop-maintenance/` | Editing source SOP/help/GitHub files unless Victor explicitly requests that exact edit |
