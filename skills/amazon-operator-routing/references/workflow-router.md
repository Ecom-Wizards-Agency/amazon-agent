...
| Product Opportunity Explorer / OEI / POE export | `amazon-opportunity-explorer`, Seller Help, MAG SOPs | `Product Opportunity Explorer`, `Opportunity Explorer`, `OEI`, `POE`, `Niche Scout`, `image strategy`, `product strategy` | Seller Central > Product Opportunity Explorer; repo-native extractor and formatter when export is needed | Modifying extractor scripts, publishing recommendations, changing listings/assets |
| SOP draft or verified SOP correction | `amazon-sop-maintenance`, source SOP/help file, current docs/UI evidence | `/create-sop`, `/fix-sop`, `outdated SOP`, `broken SOP link`, `wrong SOP steps`, `new SOP draft` | New drafts under `sop-drafts/`; verified correction notes under `sop-updates/`; local evidence under ignored `evidence/{client-or-brand-or-general}/sop-maintenance/` | Pushing source changes unless the operator explicitly asks to push |
...
| Daily account health check / sweep | `amazon-account-health-check`, alert source such as SellerSonar, Seller Central | `daily account health`, `SellerSonar`, `Featured Offer`, `Policy Compliance`, `Performance Notifications`, `action queue` | Local findings-ledger re-check first, configured alert source, Seller Central Account Health/Homepage, action-queue post in the configured daily update destination, disposition-mapped task creates/updates in the optional task tracker | Slack/task writes unless requested or configured; completing/closing tasks; appeals, acknowledgements, support contact, listing/shipment/account changes |
...
- Recommended next action, clearly marking anything that requires the operator confirmation.
