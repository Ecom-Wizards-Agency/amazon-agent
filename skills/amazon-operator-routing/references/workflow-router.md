...
| SOP bug report, SOP draft, or verified SOP correction | `amazon-sop-maintenance`, source SOP/help file, current docs/UI evidence | `/bug`, `/create-sop`, `/fix-sop`, `outdated SOP`, `broken SOP link`, `wrong SOP steps`, `new SOP draft` | Local report/draft under `output/{client-or-brand-or-general}/sop-maintenance/`; verified correction notes under `sop-updates/` | Pushing source changes unless Victor explicitly asks to push |
