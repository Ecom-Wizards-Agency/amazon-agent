...
Do not start Creator Connections from ~~`https://advertising.amazon.com/choose-account?destination=/bi`~~. That direct link can hide many accounts and only show a partial account chooser, so it is unreliable for selecting Swissker/SwissKlip or other client accounts.
