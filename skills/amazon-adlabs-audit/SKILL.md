...
- **`analyze(brand_spend_leak_detection)` substring-matches `brand_name`**, so it silently misses misspellings that do not contain the root (`brand_name="acme"` misses "accme"). Scan variants manually over the full search-term set before trusting its total.
