...
description: Check keyword-level competitors in Amazon Brand Analytics Search Query Performance (SQP). Trigger on "SQP competitor check" or when per-query competitor ASIN shares/prices are needed (AdLabs MCP only exposes our own share). Use when the operator asks to find, compare, or benchmark the top ASINs for one or more exact search queries against a selected ASIN, including query volume, impressions, impression share, clicks, click share, brand, title, and median price. Read-only.
---

# Amazon SQP Competitor Check

Browser: CDP (scripted capture via `tools/sc-sqp-competitor/`; manual UI walk only as fallback).
