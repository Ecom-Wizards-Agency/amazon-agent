...
- **Completeness gate (mandatory since 01.09.2026):** page-level inventory APIs (the Manage Inventory GraphQL sweep) silently omit active FBA listings; on 01.09.2026 this hid the top four sellers on Svens Island in both marketplaces and produced "0 units to send" plans while ~5,800 units were needed, and live checks confirmed missed FBA offers on every other configured account. Before trusting any plan, cross-check the Business Report's selling ASINs against the FBA inventory rows: any seller missing from the FBA data must be verified (search-scoped recovery or report file) and either recovered as an FBA offer or explained as FBM-only. Prefer the downloaded FBA inventory report file over page APIs wherever available, because a report enumerates every SKU in one file and row counts are verifiable; a "quiet" plan from an unverified page read is not evidence of no need.
- Report quantities in whole shipping cartons wherever the shared profile carries `carton_units`/`carton_specs` ("Send 25 boxes × 100 = 2,500 units"), and record newly learned per-ASIN carton data back to the shared profile in a supervised session instead of leaving it in run outputs.
