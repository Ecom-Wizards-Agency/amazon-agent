...
4. Run `build_launch_strategy.py --config <config> --preflight`. Resolve fatal source/configuration errors. Leave unavailable facts empty and label the result directional. Before writing either deliverable, `--build` separately preflights the installed artifact runtime through the company library. It uses Node on PATH or `--node`/`AMAZON_LAUNCH_NODE`; optional `--node-modules` selects an installed runtime. A package failure must not leave a new document/workbook pair partially generated. This builder does not require a vendor workspace dependency loader.
