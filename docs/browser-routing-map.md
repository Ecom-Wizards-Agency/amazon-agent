...
| Ad/sales audit | `amazon-audit` (`/amazon-audit`) | Mixed | AdLabs + DataDive MCP on the managed path; SQP + Business Report + ads bulk over CDP on the prospect path; live creative capture over CDP on both | Read-only. Workbook + narrative build is local. |
