...
The eventual monthly-report skill should orchestrate both and own the final
report document, workbook delivery, visual QA, and cross-source reconciliation.

## 2. Required outputs

Every completed brand run must deliver:

- One client-ready A4 monthly report as a branded native Google Doc.
- One exact audit workbook for that brand and reporting window, delivered as a
  native Google Sheet.
- Reproducible source/build files retained locally for corrections.
- A short source-and-window ledger used during QA.

Never deliver only the report. The audit workbook accompanies every brand
report. Both deliver together into the client's `<Client> - Shared/<Reports>/`
Drive folder through `tools/gdrive-deliver/deliver.py`, using the standard
`YYYY-MM-DD_<Client>_<Market>_<Artifact>_v<N>` delivery filename. No PDF is
rendered anywhere; whoever needs one downloads it from the delivered Doc.
Intermediaries and the source ledger stay in `output/{client}/reporting/`.
...
Run numerical, source, formula, and visual QA. Deliver the report document and
audit workbook only after every mandatory gate passes.
...
When one client report covers multiple marketplaces, use the approved
combined-report structure established during the pilot:
...
### Three-segment brands

A brand whose persisted configuration declares semi-branded terms always uses
three traffic segments:

- Branded
- Semi-Branded
- Non-Branded

Never collapse a three-segment brand to two segments.
...
### Brand-restricted root-cause and corrective-action package

A root-cause package is approved for one specific brand and registered with an
`allowed_brands` gate. Never include it for another brand unless the user
explicitly asks for a root-cause package for that brand.
...
- Confirm the Semi-Branded segment for three-segment brands.
...
- Confirm brand-restricted root-cause content does not leak into another brand.
- Confirm no invented goals, actions, causes, or source claims.
- Search for banned/internal phrases: `TLDR`, `Audit x Monthly Report`,
  `client-facing`, internal template nicknames (any `<name>-style` label), and
  unintended personal names.

### Visual gate

- Render every page of the built document and inspect it at readable scale,
  again after native Google conversion.
...
4. Extend the reusable builders for the workbook, charts, tables, document
   rendering, and visual QA without creating brand-specific forks.
5. Route audit work to `amazon-audit` and report fetching to
   `amazon-reporting` instead of duplicating those implementations.
6. Add regression fixtures from approved client runs; the fixtures stay in the
   client workspace, never in this repo.
