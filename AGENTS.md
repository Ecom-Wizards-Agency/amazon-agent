...
- `amazon-creator-connections`: Creator Connections campaign preparation, status-filtered inbox audits, identity-safe campaign tracker updates, qualification, operator-authorized replies and MCF fulfillment, Slack-helper reporting, tracker gaps, reconciliation, and explanation.
...
Four stop-gates: **sending a creator message**, **publishing a campaign**, **creating an MCF order**, and **posting to Slack** each need the operator's explicit approval of that exact action in the current chat or a matching `_local/local-permissions.md` standing permission. Creator Connections Slack posts also require the configured posting helper; the Slack connector is read-only or for an operator-requested personal draft.
