...
The destination decides how the file gets in: a **folder path** goes over the Drive for Desktop mount with no size limit, a **folder id or Drive URL** goes over the API and needs no mount at all, capped at 5 MB. Conversion itself needs an API that can set the target mimeType, which neither the mount nor the Drive MCP's `copy_file` offers, so it runs through Composio. Set a machine up once with `python3 tools/gdrive-deliver/setup_google.py`, which links `googledrive`, `googledocs` and `googlesheets` in one run. Without it the mount route still stages the file and prints the browser steps (open the link it gives you, `File > Save as Google Docs/Sheets`, bin the Office file). Same importer either way. On the mount route the script reads the owning account off the mount path and refuses to deliver if Composio is linked as anybody else, so a file can never be created under the wrong person's account. Composio API keys are per-person and must never be shared between machines.
