...
source_url: "https://sellercentral.amazon.com/help/hub/reference/G40571"
...
Note:
Amazon assumes the risk of buyer non-payment for every order in the FBM Unshipped orders report, and for every order that can be confirmed or canceled on your Manage orders page.
Why is an order pending?
...
The order is a business order and is pending approval from the customer. Business customers may take up to seven days from the order date to approve purchases and Business Prime Medium and Enterprise customers may take up to 14 days from the order date to approve purchases. If not approved within that time, the order will be automatically canceled. For more information about business customer approvals, go to Approvals policies.
...
To initiate communication, go to Contact a buyer.

Identify a Pending status order

To identify orders with a Pending status, go to the Manage orders page in Seller Central and click the Pending tab. All Pending orders that are to be fulfilled by Amazon can be found on the View FBA orders tab. Pending orders doesn’t appear in either your Orders report or FBM Unshipped orders report. However, your inventory will be updated to reflect the order. For more information, see Manage your orders.
...
No. Once the payment method has been verified, the Confirm shipment and Cancel order buttons will become available in Manage orders. In addition, the order will appear in both the Orders report and the FBM Unshipped orders report. At this point, the order should be shipped, or canceled, if necessary.
