...
**Client-agnostic.** Nothing is product-specific — every run is driven by a per-client config. To start a new client/product/market, copy `config.TEMPLATE.json` and follow `NEW-CLIENT.md`. The `config.sheko-*` files are just worked examples, not the tool's identity.
