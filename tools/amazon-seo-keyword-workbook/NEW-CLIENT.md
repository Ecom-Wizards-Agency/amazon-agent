...
- **Prelaunch/untracked research**: when no own ranking observation exists, set `ranking_context` to `{ "mode": "unavailable", "reason": "...", "evidence_file": "..." }`. Evidence JSON must contain matching `asin`, `marketplace`, `status` (`prelaunch` or `inactive-untracked`), `verified_at`, and existing `evidence_files` paths. Use an empty ASIN only for a product awaiting catalog creation. Disable `triage.semantic_opportunity.include_if_anchor_not_ranking`. This mode visibly marks rankings unavailable; it never substitutes a competitor or invents an own-ASIN column. All other QA gates remain required.
