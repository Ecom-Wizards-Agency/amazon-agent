...
test('pre-submit catalog read releases context then reacquires exact import target exclusively',async()=>{
 ...
 const input={task_key:'stable-job',complete_task:true,plan_hash:'a'.repeat(64),receipt_path:'/tmp/op/receipt.json',plan:{operation_id:'bound',account:{seller_id:'SELLER',marketplace_id:'MARKET'},body:{sku_asins:{a:'B000000001'},image_before_rows:{a:{asin:'B000000001'}}}}};
 const fresh={status:'collected'};
 const deps={release:async(page,options)=>{assert.equal(page,original);assert.equal(options.outcome,'handoff');calls.push('release');},collect:async request=>{assert.deepEqual(calls,['release']);assert.deepEqual(request.targets,[{sku:'a',asin:'B000000001'}]);assert.equal(request.task_key,'stable-job');assert.equal(request.operation_id,'bound');assert.equal(request.complete_task,undefined);calls.push('collect');return fresh;},acquire:async spec=>{assert.deepEqual(calls,['release','collect']);assert.equal(spec.expectedTargetId,original.targetId);assert.equal(spec.taskId,(await import('../../browserctl/task-tabs.mjs')).taskIdFor('amazon-operations','stable-job'));assert.equal(spec.exclusiveContext,true);calls.push('acquire');return resumed;}};
