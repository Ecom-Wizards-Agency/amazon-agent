...
Authentication states are returned as structured, non-secret statuses: `login_required`, `password_required`, `totp_required`, and `human_challenge`. Authentication failures never create screenshots. The governing repository policy requires human completion of passwords and MFA, so `auto_reauth` remains disabled.
